...
restore_stage=0.5
second_stage=0.5
critical_lower_limit=-80.
noise_limit=0.06    #noise_limit must > noise_limit_add
noise_limit_add=0.05
is_noise=0
...
def Learning(Tragectory, MoveDirection, Rwrd_lst, MzIndex_lst, walk_average, walk, is_noise_arr):
...
#"""+(abs(walk_average-walk)/walk)*0.2*const"""*(13**stage)/100
        ...
        if((float(walk_average-walk)/walk_average)<critical_lower_limit):
            #print "critical!!"
            break
        ...
        if(dropout_rate<rand or ((punish_count<6)and(walk_average<walk)) or iss_noise==1):
        ...
        if(Rwrd_lst[index][xp][yp][direc] + stage*(gamma_rate*(float(walk_average-walk)/walk_average))>0):    #업데이트 해서 음수가 되지 않는다면
            Rwrd_lst[index][xp][yp][direc] = Rwrd_lst[index][xp][yp][direc] + stage*(gamma_rate*(float(walk_average-walk)/walk_average))
...
while (1):
    ...
    if (iteration % 200 == 1 and iteration > 1):
        if(before_avg<walk_average):
            if(noise_limit+noise_limit_add>0.35 and (noise_limit-noise_limit_add>0)):
                noise_limit=noise_limit-noise_limit_add
            else:
                noise_limit=noise_limit+noise_limit_add
            #stage = stage / decay_rate
        elif(before_avg>walk_average and (noise_limit-noise_limit_add>0)):
            noise_limit-=noise_limit_add

        stage = stage * decay_rate

        before_avg=walk_average
    ...
    Learning(Tragectory, MoveDirection, Rwrd_lst, MzIndex_lst, walk_average, walk, is_noise_arr)
    Tragectory = []
    MoveDirection = []
    is_noise_arr=[]
    MzIndex_lst=[0]
